...
class ShowFoodWithIngredientTagsTranslatedController extends Controller
{
	...
	function CheckPaginate($query,$per_page,$page){
		...
		if(isset($per_page))
			return view('ingredientTagPage',['all'=>[\Illuminate\Http\JsonResponse::create(\App\Http\Controllers\CostumPaginateController::paginate(collect($query),$per_page))]]);
		else
			return view('ingredientTagPage',['all'=>['data'=>$query]]);
	}
	...
	function GetString($query,$lang){
		...
		for($i =0; $i < count($query); $i++){
			...
			array_push($string,['title'=>$query[$i]->title,'description'=>$query[$i]->description,'tag'=>$foodTags,'ingredient'=>$foodIngredient]);
		}
