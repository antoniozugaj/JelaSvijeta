...
class ShowFoodWithTagsIngredientCategoryController extends Controller
{
			...
			if($category=="-"){//category
					...
					if($validTags){
						...
						$foodWithCategory = Food::where('category_id',$category)->where('status','created')
							->select('food.id','foods.title','foods.description')->get();	
	...
	function GetString($query){
		...
		for($i =0; $i < count($query); $i++){
			...
			$foodIngredient = Ingredient::join('food_ingredient','ingredients.id','=','food_ingredient.ingredient_id')
				...
				->select('ingredients.id','ingredients.title')
				->get(); 
